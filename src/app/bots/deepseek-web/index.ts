...
export class DeepSeekWebBot extends AbstractBot {
  ...
  async doSendMessage(params: SendMessageParams): Promise<void> {
    ...
    const resp = await fetch('https://chat.deepseek.com/api/chat', {
      method: 'POST',
    ...
    if (!resp.ok) {
      if (resp.status === 401 || resp.status === 403) {
        ...
        throw new ChatError(
          'There is no logged-in DeepSeek account in this browser.',
          ErrorCode.DEEPSEEK_WEB_UNAUTHORIZED,
        )
      }
      if (resp.status === 404) {
        // Conversation not found, reset and try again
        this.conversationContext = undefined
        throw new ChatError('Conversation not found. Please try again.', ErrorCode.DEEPSEEK_WEB_CONVERSATION_NOT_FOUND)
      }
      throw new ChatError(`DeepSeek API error: ${resp.status}`, ErrorCode.UNKOWN_ERROR)
    }
