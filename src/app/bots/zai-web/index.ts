...
export class ZAIWebBot extends AbstractBot {
  ...
  async doSendMessage(params: SendMessageParams): Promise<void> {
    if (!(await requestHostPermission('https://chat.z.ai/'))) {
      throw new ChatError('Missing chat.z.ai permission', ErrorCode.MISSING_HOST_PERMISSION)
    }
    ...
    const resp = await fetch('https://chat.z.ai/api/chat', {
      method: 'POST',
    ...
    if (!resp.ok) {
      if (resp.status === 401 || resp.status === 403) {
        ...
        throw new ChatError('There is no logged-in Z.AI account in this browser.', ErrorCode.ZAI_WEB_UNAUTHORIZED)
      }
      if (resp.status === 404) {
        // Conversation not found, reset and try again
        this.conversationContext = undefined
        throw new ChatError('Conversation not found. Please try again.', ErrorCode.ZAI_WEB_CONVERSATION_NOT_FOUND)
      }
      throw new ChatError(`Z.AI API error: ${resp.status}`, ErrorCode.UNKOWN_ERROR)
    }
    ...
    await parseSSEResponse(resp, (message) => {
      console.debug('zai sse message', message)
      try {
